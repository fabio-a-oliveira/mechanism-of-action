...
# selection of PCs, with range limiting
# competition submission ??? - 2020-10-15
...
train$pca.features <-                                                                                                   # remove ctl vehicles
...
for (mec in 1:numMechanisms) {
  ...
  y <- train$targets[-train$heldOut,] %>% filter(!(sig_id %in% ctl_vehicle)) %>%
...
# alternative method - all mechanisms at once, each mechanism as a factor - DOES NOT WORK WITH GLM!!!
# top_pcs <- paste("PC", 1:numPrincipalComponents, sep = "")
# y <- train$targets %>% 
...
# x <- y %>% 
#   left_join(train$features %>% select(-cp_type,-cp_time,-cp_dose),
#             by = "sig_id") %>% 
#   select(-mechanism)
# 
# fit <- train(method = method,
...
#                 positive = "1")
# 
# Calculate quantiles of score on held out samples ---------------------------------------------------------------------
...
bootstrap.scores <- replicate(1000, {
  df %>% 
...
quantile(bootstrap.scores, c(.01, .05, .10, .25, .50, .75, .90, .95, .99))
